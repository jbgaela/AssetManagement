...
class VideoCard extends Component {
    constructor(props) {
        ...
        this.state = {
            videocard: [],
            videocardInput: '',
            url: 'http://localhost:64294/api/Sizes/videocard',
            resultData: {
                currentPage: 0,
                success: false,
                total: 0,
    ...
    loadData() {
        ...
        axios.get(this.state.url + "/?ShowAll=true",{headers:headers}).then(result => {
            if (result.statusText === "OK") {
                var videocard = result.data.list;
                this.setState({
                    videocard: videocard,
                    resultData: {
            ...
            <Grid>
                <Row className="show-grid">
                            ...
                            <Panel.Body>
                                ...
                                <FormGroup controlId="formHorizontal_Name" validationState={this.state.isInputValid} >
                                    <FieldGroup
                                        ...
                                        placeholder="Please enter video card size."                                        
                                        inputRef={(ref) => { this.videocardName = ref }}                                       
                                    />
                                </FormGroup>
                    ...
                    <Col xs={12} md={7}>
                        <Panel bsStyle="default">
                            <Panel.Heading>
                                <Panel.Title componentClass="h3">List of videocards
                                </Panel.Title>
                            </Panel.Heading>
                            <Panel.Body>
                                <BootstrapTable data={this.state.videocard} 
                                    options={this.pagingTable(this.state.videocard.length)}
                                    striped hover>
                                    <TableHeaderColumn dataField='id' isKey={true} width="80" >ID</TableHeaderColumn>
                                    <TableHeaderColumn dataField='size' width="160">Videocard Size</TableHeaderColumn>
                                    <TableHeaderColumn
                                        dataField='button'
                                        width="100"
